            ...
            <div data-role='content' style="background-color: white;">
                <div data-role="controlgroup" data-type="horizontal">
					...
					<c:if test="${sessionScope.mytripCode ne 0}">
						<a class="mytrip-detail-btn" id="addmytrip-tourist" data-role="button" style="background-color: white; color: black;" href="../addmytrip_list/${basicInfo.contentid }">내 여행지 추가</a>
					</c:if>
					<c:if test="${sessionScope.mytripCode eq 0}">
						<a class="mytrip-detail-btn" id="addmytrip-tourist" onclick="addmytriptourist()" data-role="button" style="background-color: white; color: black;" href="#">내 여행지 추가</a>
					</c:if>
				</div>
				
				<div id="mytrip-tourist" style="display: none; position: fixed; top: 10%; width: 80%; left: 5%; z-index: 100; background-color: white; border: 5px solid #F05562; border-radius: 20px; padding: 15px;">
					<h4></h4>
					<ul id="mytriptourist" data-role="listview" data-inset="true"></ul>
					<div></div>
				</div>
				...
				<script type="text/javascript">
						...
						if(userInfo == '') {
							...
							$.ajax({ 
					            ...
					            success : function(data){
					            	...
					            	$('#mytrip-tourist div').html('');
					            	if(data.length != 0) {
					            		$('#mytrip-tourist h4').html(usernicname + '님의 여행목록');
					            		$('#mytriptourist').html("<li class='ui-li-has-thumb ui-first-child ui-last-child' data-icon='plus'>"
					            		...
					            		$('#mytriptourist').append('<button onclick="removeTripPop()" class="ui-btn ui-shadow ui-corner-all ui-last-child" style="background-color: #F05562; color: white;">취소</button>');
								        $('#mytriptourist').listview('refresh');
					            	} else {
					            		$('#mytrip-tourist').html('<h4>' + usernicname + '님의 여행목록이 없습니다.</h4>'
					...
					function tripDetail(paramdata) {
						$.ajax({ 
				            ...
				            success : function(data){
				            	$('#mytrip-tourist div').html('');
				            	$('#mytrip-tourist h4').html('여행지 추가');
				            	$('#mytriptourist').html('<li class="ui-first-child ui-last-child" data-icon="plus">'
										+ '<a onclick="addTrip(' + data[0]['code'] + ', \'' + data[0]['tripdate'] + '\')" href="#">'
								        + '<h2>여행 ' + data[0]['daynum'] + '일차 - ' + data[0]['tripdate'] + '</h2></a></li>');
								for(var i=1; i<data.length; i++) {
									$('#mytriptourist').append('<li class="ui-first-child ui-last-child" data-icon="plus">'
					...
					function addTrip(code, tripdate) {
						$('#mytrip-tourist h4').html('<h4>여행지에 추가하시겠습니까?</h4>');
						$('#mytriptourist').html('');
						$('#mytriptourist').listview('refresh');
						$('#mytrip-tourist div').html('<button onclick="addTrip2(' + code + ', \'' + tripdate + '\')">추가</button><button onclick="removeTripPop()">취소</button>');
					};
				...
				<div style="padding-left: 10px; padding-right: 10px; position: relative; top: -48px;">
					...
					<div style="width: 100%; border-bottom: 3px solid #e9e9e9; margin-top: 10px; padding-left: 5px; padding-bottom: 5px; margin-bottom: 10px;">
						<a id="touristdetaila" href="#" style="text-decoration: none; color: #F05562; font-size: 16px; font-weight: bold;">
							관광지 상세정보 +
						</a>
					</div>
					
					<div id="touristdetailpop" style="display: none; padding-left: 5px; margin-bottom: 20px;">
						<table style="font-size: 14px;">
							<tbody>
								<c:if test="${detailInfo.expagerange ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">체험연령</th>
										<td colspan="3">${detailInfo.expagerange }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.infocenter ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">문의안내</th>
										<td colspan="3">${detailInfo.infocenter }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.parking ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">주차시설</th>
										<td colspan="3">${detailInfo.parking }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.restdate ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">쉬는날</th>
										<td colspan="3">${detailInfo.restdate }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.useseason ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">이용시기</th>
										<td colspan="3">${detailInfo.useseason }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.usetime ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">이용시간</th>
										<td colspan="3">${detailInfo.usetime }</td>
									</tr>
								</c:if>							
							</tbody>
						</table>
					...
					<script type="text/javascript">
						var flag = 1;
						$('#touristdetaila').click(function() {
							if(flag == 1) {
								flag = 0;
								$('#touristdetailpop').show();
								$('#touristdetaila').html('관광지 상세정보 -');
							}
							else {
								flag = 1;
								$('#touristdetailpop').hide();
								$('#touristdetaila').html('관광지 상세정보 +');
							}
						});
					</script>
					...
					<div style="width: 100%; background-color: white; border-bottom: 3px solid #e9e9e9; margin-top: 20px; margin-bottom: 10px;">
			        	<table style="width: 100%;">
			        		<tr>
			        			...
			        			<td align="right" style="padding-right: 5px;"><a href="#" onclick="showtouristImg()" style="text-decoration: none; color: gray; font-size: 14px;">사진추가</a></td>
			        		</tr>
			        	</table>
			        </div>
			        <div id="touristupimg" class="img-slider" style="border: none; margin-bottom: 20px;">
			        ...
			        <div id="touristpop" style="border: 2px solid #F05562; border-radius: 10px; display: none; width: 90%; position: fixed; top: 30%; left: 5%; z-index: 100;">
				        <form id="fileFormtourist" action="uploadImg" method="post" enctype="multipart/form-data" data-ajax="false" style="background-color: white;">
							<div style="padding: 10px;">
								<input type="hidden" name="contentid" value="${basicInfo.contentid }"/>
								<input style="z-index: 100" type="file" id="uploadtourist" name="file"/>
								...
								<a href="#" class="ui-btn ui-btn-inline" style="color: white; background-color: #F05562; box-sizing: border-box; display: inline-block; width: 45%; z-index: 100" onclick="hidetouristImg()">취소</a>
							</div>
						</form>
				    </div>
		
					<script type="text/javascript">
				        function showtouristImg() {
				        	if(userInfo == '') {
								alert('로그인 후 사용하실 수 있습니다.');
							} else {
				        		$('#touristpop').show();
