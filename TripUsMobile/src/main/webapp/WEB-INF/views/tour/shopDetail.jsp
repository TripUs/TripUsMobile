            ...
            <div data-role='content' style="background-color: white;">
                <div data-role="controlgroup" data-type="horizontal">
					...
					<c:if test="${sessionScope.mytripCode ne 0}">
						<a class="mytrip-detail-btn" id="addmytrip-shop" data-role="button" style="background-color: white; color: black;" href="../addmytrip_list/${basicInfo.contentid }">내 여행지 추가</a>
					</c:if>
					<c:if test="${sessionScope.mytripCode eq 0}">
						<a class="mytrip-detail-btn" id="addmytrip-shop" onclick="addmytripshop()" data-role="button" style="background-color: white; color: black;" href="#">내 여행지 추가</a>
					</c:if>
				</div>
				
				<div id="mytrip-shop" style="display: none; position: fixed; top: 10%; width: 80%; left: 5%; z-index: 100; background-color: white; border: 5px solid #F05562; border-radius: 20px; padding: 15px;">
					<h4></h4>
					<ul id="mytripshop" data-role="listview" data-inset="true"></ul>
					<div></div>
				</div>
				...
				<script type="text/javascript">
						...
						if(userInfo == '') {
							...
							$.ajax({ 
					            ...
					            success : function(data){
					            	...
					            	$('#mytrip-shop div').html('');
					            	if(data.length != 0) {
					            		$('#mytrip-shop h4').html(usernicname + '님의 여행목록');
					            		$('#mytripshop').html("<li class='ui-li-has-thumb ui-first-child ui-last-child' data-icon='plus'>"
					            		...
					            		$('#mytripshop').append('<button onclick="removeTripPop()" class="ui-btn ui-shadow ui-corner-all ui-last-child" style="background-color: #F05562; color: white;">취소</button>');
								        $('#mytripshop').listview('refresh');
					            	} else {
					            		$('#mytrip-shop').html('<h4>' + usernicname + '님의 여행목록이 없습니다.</h4>'
					...
					function tripDetail(paramdata) {
						$.ajax({ 
				            ...
				            success : function(data){
				            	$('#mytrip-shop div').html('');
				            	$('#mytrip-shop h4').html('여행지 추가');
				            	$('#mytripshop').html('<li class="ui-first-child ui-last-child" data-icon="plus">'
										+ '<a onclick="addTrip(' + data[0]['code'] + ', \'' + data[0]['tripdate'] + '\')" href="#">'
								        + '<h2>여행 ' + data[0]['daynum'] + '일차 - ' + data[0]['tripdate'] + '</h2></a></li>');
								for(var i=1; i<data.length; i++) {
									$('#mytripshop').append('<li class="ui-first-child ui-last-child" data-icon="plus">'
					...
					function addTrip(code, tripdate) {
						$('#mytrip-shop h4').html('<h4>여행지에 추가하시겠습니까?</h4>');
						$('#mytripshop').html('');
						$('#mytripshop').listview('refresh');
						$('#mytrip-shop div').html('<button onclick="addTrip2(' + code + ', \'' + tripdate + '\')">추가</button><button onclick="removeTripPop()">취소</button>');
					};
				...
				<div style="padding-left: 10px; padding-right: 10px; position: relative; top: -48px;">
					...
					<div style="width: 100%; border-bottom: 3px solid #e9e9e9; margin-top: 10px; padding-left: 5px; padding-bottom: 5px; margin-bottom: 10px;">
						<a id="shopdetaila" href="#" style="text-decoration: none; color: #F05562; font-size: 16px; font-weight: bold;">
							쇼핑 상세정보 +
						</a>
					</div>
					
					<div id="shopdetailpop" style="display: none; padding-left: 5px; margin-bottom: 20px;">
						<table style="font-size: 14px;">
							<tbody>
								<c:if test="${detailInfo.saleitem ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">판매품목</th>
										<td colspan="3">${detailInfo.saleitem }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.saleitemcost ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">가격</th>
										<td colspan="3">${detailInfo.saleitemcost }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.shopguide ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">매장안내</th>
										<td colspan="3">${detailInfo.shopguide }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.fairday ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">장서는날</th>
										<td colspan="3">${detailInfo.fairday }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.opentime ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">영업시간</th>
										<td colspan="3">${detailInfo.opentime }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.infocentershopping ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">문의안내</th>
										<td colspan="3">${detailInfo.infocentershopping }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.parkingshopping ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">주차시설</th>
										<td colspan="3">${detailInfo.parkingshopping }</td>
									</tr>
								</c:if>								
							</tbody>
						</table>
					...
					<script type="text/javascript">
						var flag = 1;
						$('#shopdetaila').click(function() {
							if(flag == 1) {
								flag = 0;
								$('#shopdetailpop').show();
								$('#shopdetaila').html('쇼핑 상세정보 -');
							}
							else {
								flag = 1;
								$('#shopdetailpop').hide();
					...
					<div style="width: 100%; background-color: white; border-bottom: 3px solid #e9e9e9; margin-top: 20px; margin-bottom: 10px;">
			        	<table style="width: 100%;">
			        		<tr>
			        			...
			        			<td align="right" style="padding-right: 5px;"><a href="#" onclick="showshopImg()" style="text-decoration: none; color: gray; font-size: 14px;">사진추가</a></td>
			        		</tr>
			        	</table>
			        </div>
			        <div id="shopupimg" class="img-slider" style="border: none; margin-bottom: 20px;">
			        ...
			        <div id="shoppop" style="border: 2px solid #F05562; border-radius: 10px; display: none; width: 90%; position: fixed; top: 30%; left: 5%; z-index: 100;">
				        <form id="fileFormshop" action="uploadImg" method="post" enctype="multipart/form-data" data-ajax="false" style="background-color: white;">
							<div style="padding: 10px;">
								<input type="hidden" name="contentid" value="${basicInfo.contentid }"/>
								<input style="z-index: 100" type="file" id="uploadshop" name="file"/>
								...
								<a href="#" class="ui-btn ui-btn-inline" style="color: white; background-color: #F05562; box-sizing: border-box; display: inline-block; width: 45%; z-index: 100" onclick="hideshopImg()">취소</a>
							</div>
						</form>
				    </div>
