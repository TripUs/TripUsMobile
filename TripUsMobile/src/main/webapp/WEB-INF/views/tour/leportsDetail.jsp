            ...
            <div data-role='content' style="background-color: white;">
                <div data-role="controlgroup" data-type="horizontal">
					...
					<c:if test="${sessionScope.mytripCode ne 0}">
						<a class="mytrip-detail-btn" id="addmytrip-leports" data-role="button" style="background-color: white; color: black;" href="../addmytrip_list/${basicInfo.contentid }">내 여행지 추가</a>
					</c:if>
					<c:if test="${sessionScope.mytripCode eq 0}">
						<a class="mytrip-detail-btn" id="addmytrip-leports" onclick="addmytripleports()" data-role="button" style="background-color: white; color: black;" href="#">내 여행지 추가</a>
					</c:if>
				</div>
				
				<div id="mytrip-leports" style="display: none; position: fixed; top: 10%; width: 80%; left: 5%; z-index: 100; background-color: white; border: 5px solid #F05562; border-radius: 20px; padding: 15px;">
					<h4></h4>
					<ul id="mytripleports" data-role="listview" data-inset="true"></ul>
					<div></div>
				</div>
				...
				<script type="text/javascript">
						...
						if(userInfo == '') {
							...
							$.ajax({ 
					            ...
					            success : function(data){
					            	...
					            	$('#mytrip-leports div').html('');
					            	if(data.length != 0) {
					            		$('#mytrip-leports h4').html(usernicname + '님의 여행목록');
					            		$('#mytripleports').html("<li class='ui-li-has-thumb ui-first-child ui-last-child' data-icon='plus'>"
					            		...
					            		$('#mytripleports').append('<button onclick="removeTripPop()" class="ui-btn ui-shadow ui-corner-all ui-last-child" style="background-color: #F05562; color: white;">취소</button>');
								        $('#mytripleports').listview('refresh');
					            	} else {
					            		$('#mytrip-leports').html('<h4>' + usernicname + '님의 여행목록이 없습니다.</h4>'
					...
					function tripDetail(paramdata) {
						$.ajax({ 
				            ...
				            success : function(data){
				            	$('#mytrip-leports div').html('');
				            	$('#mytrip-leports h4').html('여행지 추가');
				            	$('#mytripleports').html('<li class="ui-first-child ui-last-child" data-icon="plus">'
										+ '<a onclick="addTrip(' + data[0]['code'] + ', \'' + data[0]['tripdate'] + '\')" href="#">'
								        + '<h2>여행 ' + data[0]['daynum'] + '일차 - ' + data[0]['tripdate'] + '</h2></a></li>');
								for(var i=1; i<data.length; i++) {
									$('#mytripleports').append('<li class="ui-first-child ui-last-child" data-icon="plus">'
					...
					function addTrip(code, tripdate) {
						$('#mytrip-leports h4').html('<h4>여행지에 추가하시겠습니까?</h4>');
						$('#mytripleports').html('');
						$('#mytripleports').listview('refresh');
						$('#mytrip-leports div').html('<button onclick="addTrip2(' + code + ', \'' + tripdate + '\')">추가</button><button onclick="removeTripPop()">취소</button>');
					};
				...
				<div style="padding-left: 10px; padding-right: 10px; position: relative; top: -48px;">
					...
					<div style="width: 100%; border-bottom: 3px solid #e9e9e9; margin-top: 10px; padding-left: 5px; padding-bottom: 5px; margin-bottom: 10px;">
						<a id="leportsdetaila" href="#" style="text-decoration: none; color: #F05562; font-size: 16px; font-weight: bold;">
							레포츠 상세정보 +
						</a>
					</div>
					
					<div id="leportsdetailpop" style="display: none; padding-left: 5px; margin-bottom: 20px;">
						<table style="font-size: 14px;">
							<tbody>
								<c:if test="${detailInfo.usefeeleports ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">입장료</th>
										<td colspan="3">${detailInfo.usefeeleports }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.usetimeleports ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">이용시간</th>
										<td colspan="3">${detailInfo.usetimeleports }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.reservation ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">예약안내</th>
										<td colspan="3">${detailInfo.reservation }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.openperiod ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">개장안내</th>
										<td colspan="3">${detailInfo.openperiod }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.expagerangeleports ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">가능연령</th>
										<td colspan="3">${detailInfo.expagerangeleports }</td>
									</tr>
								</c:if>
								<c:if test="${detailInfo.parkingleports ne '데이터 없음' }">
									<tr>
										<th width="30%" style="text-align: left;">주차시설</th>
										<td colspan="3">${detailInfo.parkingleports }</td>
									</tr>
								</c:if>
							</tbody>
						</table>
					...
					<script type="text/javascript">
						var flag = 1;
						$('#leportsdetaila').click(function() {
							if(flag == 1) {
								flag = 0;
								$('#leportsdetailpop').show();
								$('#leportsdetaila').html('레포츠 상세정보 -');
							}
							else {
								flag = 1;
								$('#leportsdetailpop').hide();
								$('#leportsdetaila').html('레포츠 상세정보 +');
							}
						});
					</script>
					...
					<div style="width: 100%; background-color: white; border-bottom: 3px solid #e9e9e9; margin-top: 20px; margin-bottom: 10px;">
			        	<table style="width: 100%;">
			        		<tr>
			        			...
			        			<td align="right" style="padding-right: 5px;"><a href="#" onclick="showleportsImg()" style="text-decoration: none; color: gray; font-size: 14px;">사진추가</a></td>
			        		</tr>
			        	</table>
			        </div>
			        <div id="leportsupimg" class="img-slider" style="border: none; margin-bottom: 20px;">
			        ...
			        <div id="leportspop" style="border: 2px solid #F05562; border-radius: 10px; display: none; width: 90%; position: fixed; top: 30%; left: 5%; z-index: 100;">
				        <form id="fileFormleports" action="uploadImg" method="post" enctype="multipart/form-data" data-ajax="false" style="background-color: white;">
							<div style="padding: 10px;">
								<input type="hidden" name="contentid" value="${basicInfo.contentid }"/>
								<input style="z-index: 100" type="file" id="uploadleports" name="file"/>
								...
								<a href="#" class="ui-btn ui-btn-inline" style="color: white; background-color: #F05562; box-sizing: border-box; display: inline-block; width: 45%; z-index: 100" onclick="hideleportsImg()">취소</a>
							</div>
						</form>
				    </div>
		
					<script type="text/javascript">
				        function showleportsImg() {
				        	if(userInfo == '') {
								alert('로그인 후 사용하실 수 있습니다.');
							} else {
				        		$('#leportspop').show();
