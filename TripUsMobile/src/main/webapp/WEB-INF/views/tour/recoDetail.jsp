            ...
            <div data-role='content' style="background-color: white;">
                <div data-role="controlgroup" data-type="horizontal">
					...
					<c:if test="${sessionScope.mytripCode ne null}">
						<a class="mytrip-detail-btn" id="addmytrip-course" data-role="button" style="background-color: white; color: black;" href="../addmytrip_list/${basicInfo.contentid }">내 여행지 추가</a>
					</c:if>
					<c:if test="${sessionScope.mytripCode eq null}">
						<a class="mytrip-detail-btn" id="addmytrip-course" onclick="addmytripcourse()" data-role="button" style="background-color: white; color: black;" href="#">내 여행지 추가</a>
					</c:if>
				</div>
				
				<div id="mytrip-course" style="display: none; position: fixed; top: 10%; width: 80%; left: 5%; z-index: 100; background-color: white; border: 5px solid #F05562; border-radius: 20px; padding: 15px;">
					<h4></h4>
					<ul id="mytripcourse" data-role="listview" data-inset="true"></ul>
					<div></div>
				</div>
				...
				<input id="sigungucode" type="hidden" value="${basicInfo.sigungucode }">
				...
				<script type="text/javascript">
						...
						if(userInfo == '') {
							...
							$.ajax({ 
					            ...
					            success : function(data){
					            	...
					            	$('#mytrip-course div').html('');
					            	if(data.length != 0) {
					            		$('#mytrip-course h4').html(usernicname + '님의 여행목록');
					            		$('#mytripcourse').html("<li class='ui-li-has-thumb ui-first-child ui-last-child' data-icon='plus'>"
					            		...
					            		$('#mytripcourse').append('<button onclick="removeTripPop()" class="ui-btn ui-shadow ui-corner-all ui-last-child" style="background-color: #F05562; color: white;">취소</button>');
								        $('#mytripcourse').listview('refresh');
					            	} else {
					            		$('#mytrip-course').html('<h4>' + usernicname + '님의 여행목록이 없습니다.</h4>'
					...
					function tripDetail(paramdata) {
						$.ajax({ 
				            ...
				            success : function(data){
				            	$('#mytrip-course div').html('');
				            	$('#mytrip-course h4').html('여행지 추가');
				            	$('#mytripcourse').html('<li class="ui-first-child ui-last-child" data-icon="plus">'
										+ '<a onclick="addTrip(' + data[0]['code'] + ', \'' + data[0]['tripdate'] + '\')" href="#">'
								        + '<h2>여행 ' + data[0]['daynum'] + '일차 - ' + data[0]['tripdate'] + '</h2></a></li>');
								for(var i=1; i<data.length; i++) {
									$('#mytripcourse').append('<li class="ui-first-child ui-last-child" data-icon="plus">'
					...
					function addTrip(code, tripdate) {
						$('#mytrip-course h4').html('<h4>여행지에 추가하시겠습니까?</h4>');
						$('#mytripcourse').html('');
						$('#mytripcourse').listview('refresh');
						$('#mytrip-course div').html('<button onclick="addTrip2(' + code + ', \'' + tripdate + '\')">추가</button><button onclick="removeTripPop()">취소</button>');
					};
				...
				<div style="padding-left: 10px; padding-right: 10px; position: relative; top: -48px;">
					<div style="padding: 5px;">
						<table style="width: 100%; font-size: 14px;">
							...
							<c:if test="${detailInfo.theme ne '데이터 없음' }">
								<tr>
									<th width="30%" style="text-align: left;">코스테마</th>
									<td colspan="3">${detailInfo.theme }</td>
								</tr>
							</c:if>
							<c:if test="${detailInfo.distance ne '데이터 없음' }">
								<tr>
									<th width="30%" style="text-align: left;">코스 총거리</th>
									<td colspan="3">${detailInfo.distance }</td>
								</tr>
							</c:if>
							<c:if test="${detailInfo.schedule ne '데이터 없음' }">
								<tr>
									<th width="30%" style="text-align: left;">코스일정</th>
									<td colspan="3">${detailInfo.schedule }</td>
								</tr>
							</c:if>
							<c:if test="${detailInfo.taketime ne '데이터 없음' }">
								<tr>
									<th width="30%" style="text-align: left;">소요시간</th>
									<td colspan="3">${detailInfo.taketime }</td>
								</tr>
							</c:if>							
						</table>
					</div>
					...
					<div style="width: 100%; border-bottom: 3px solid #e9e9e9; margin-top: 10px; padding-left: 5px; padding-bottom: 5px; margin-bottom: 10px;">
						<a id="coursedetaila" href="#" style="text-decoration: none; color: #F05562; font-size: 16px; font-weight: bold;">
							여행코스 상세정보 +
						</a>
					</div>
					<div id="coursedetailpop" style="display: none; padding-left: 5px; margin-bottom: 20px;">
						...
						<c:set value="${n.index+1 }" var="day"></c:set>
							<div style="padding: 10px 10px; border: 2px solid #e9e9e9; border-radius: 10px; margin-bottom: 10px;">
								<div style="font-size: 16px; color: gray; margin-bottom: 5px;">
									<strong>${day }. <a href="../detail/${bean.subcontentid }" style="text-decoration: none; font-size: 14px; color: gray;">${bean.subname }</a></strong>
								</div>
								<div style="margin-bottom: 5px;">
									<c:if test="${bean.subdetailimg ne '데이터 없음' }">
					...
					<script type="text/javascript">
						var flag = 1;
						$('#coursedetaila').click(function() {
							if(flag == 1) {
								flag = 0;
								$('#coursedetailpop').show();
								$('#coursedetaila').html('여행코스 상세정보 -');
							}
							else {
								flag = 1;
								$('#coursedetailpop').hide();
								$('#coursedetaila').html('여행코스 상세정보 +');
							}
						});
					</script>
					...
					<div style="width: 100%; background-color: white; border-bottom: 3px solid #e9e9e9; margin-top: 20px; margin-bottom: 10px;">
			        	<table style="width: 100%;">
			        		<tr>
			        			...
			        			<td align="right" style="padding-right: 5px;"><a href="#" onclick="showcourseImg()" style="text-decoration: none; color: gray; font-size: 14px;">사진추가</a></td>
			        		</tr>
			        	</table>
			        </div>
			        <div id="courseupimg" class="img-slider" style="border: none; margin-bottom: 20px;">
			        ...
			        <div id="coursepop" style="border: 2px solid #F05562; border-radius: 10px; display: none; width: 90%; position: fixed; top: 30%; left: 5%; z-index: 100;">
				        <form id="fileFormcourse" action="uploadImg" method="post" enctype="multipart/form-data" data-ajax="false" style="background-color: white;">
							<div style="padding: 10px;">
								<input type="hidden" name="contentid" value="${basicInfo.contentid }"/>
								<input style="z-index: 100" type="file" id="uploadcourse" name="file"/>
								...
								<a href="#" class="ui-btn ui-btn-inline" style="color: white; background-color: #F05562; box-sizing: border-box; display: inline-block; width: 45%; z-index: 100" onclick="hidecourseImg()">취소</a>
							</div>
						</form>
				    </div>
		
					<script type="text/javascript">
				        function showcourseImg() {
				        	if(userInfo == '') {
								alert('로그인 후 사용하실 수 있습니다.');
							} else {
				        		$('#coursepop').show();
